...
class DrawImages(private val context: Context) {
    ...
    private fun applyTransparentOverlay(context: Context, overlay: Bitmap, segmentationResult: SegmentationResult, overlayColorResId: Int) {
        ...
        val translatedName = TranslationHelper.translate(box.clsName)

        val bounds = android.graphics.Rect()
        textPaint.getTextBounds(translatedName, 0, translatedName.length, bounds)
        ...
        canvas.drawText(translatedName, left.toFloat() + padding, top.toFloat() - padding.toFloat(), textPaint)
    }
